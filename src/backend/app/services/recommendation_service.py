...
def _add_category_weight(
    ...
    try:
        ...
        return
    category_weights[normalized_category_id] = (
        category_weights.get(normalized_category_id, 0.0) + weight
    )


def get_student_signals(student_id: str) -> dict[str, Any]:
    """Return profile signals shared by recommendations and the chatbot."""
    ...
    for row in registrations_response.data or []:
        event_id = row.get("event_id")
        if event_id:
            engaged_event_ids.add(str(event_id))
        if _normalize_status(row.get("registration_status")) == "CANCELLED":
            continue
        category_id = (row.get("events") or {}).get("category_id")
        _add_category_weight(category_weights, category_id, 2.0)

    for row in saved_events_response.data or []:
        event_id = row.get("event_id")
        if event_id:
            engaged_event_ids.add(str(event_id))
        category_id = (row.get("events") or {}).get("category_id")
        _add_category_weight(category_weights, category_id, 1.0)
