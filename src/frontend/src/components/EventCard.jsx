...
import { Clock, MapPin, QrCode, AlertTriangle, CheckCircle2 } from "lucide-react";
        ...
        <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition flex flex-col justify-between relative group">
            ...
            <div className="p-5 pt-0 flex items-center gap-2">
                {isAttended ? (
                    <button
                        disabled
                        className="w-full text-xs font-semibold py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 transition bg-emerald-50 text-emerald-700 border border-emerald-200 cursor-default"
                    >
                        <CheckCircle2 className="w-4 h-4" />
                        Đã điểm danh (Đã tham gia)
                    </button>
                ) : isAbsent ? (
                    <button
                        disabled
                    ...
                    <>
                        <button className="w-full text-xs font-semibold py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 transition shadow-sm bg-[#6D28D9] hover:bg-[#7E22CE] text-white active:scale-[0.99]">
                            <QrCode className="w-4 h-4" />
                            Xem QR Check-in
                        </button>
