...
export default function ConfirmActionDialog({ action, onCancel, onConfirm }) {
      ...
      >
        ...
        <div className="mt-7 grid grid-cols-2 gap-3">
          <button
            type="button"
            onClick={onCancel}
            className="h-11 rounded-xl border border-[#d8d0df] bg-white px-4 text-sm font-semibold text-[#51495a] transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#f8f6fa]"
          >
            Quay lại
          </button>
          <button
            type="button"
            onClick={onConfirm}
            className={`h-11 rounded-xl px-4 text-sm font-semibold text-white shadow-lg transition-all duration-200 hover:-translate-y-0.5 ${
              isApproval
                ? "bg-[#6d20df] shadow-[#6d20df]/20 hover:bg-[#5915bd]"
                : "bg-[#c92d2d] shadow-[#c92d2d]/20 hover:bg-[#ac2222]"
            }`}
          >
            {isApproval ? "Xác nhận duyệt" : "Xác nhận từ chối"}
          </button>
        </div>
      </section>
    </div>
