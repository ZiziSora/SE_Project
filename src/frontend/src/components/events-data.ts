...
export default function CreateEventForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Reset form hoặc chuyển hướng về trang dashboard nếu muốn
    }
  }
  ...
  useEffect(() => {
    async function fetchDropdownData() {
      ...
      if (categoryError) console.error('Lỗi tải danh mục:', categoryError)
  ...
  // --- LẤY GỢI Ý ĐỊA ĐIỂM TỪ SUPABASE ---
  useEffect(() => {
    async function fetchExistingLocations() {
      ...
      if (error) {
        ...
        setSuggestedLocations(uniqueLocations as string[])
      }
    }
