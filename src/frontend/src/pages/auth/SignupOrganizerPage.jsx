...
const SignupOrganizerPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setLoading(true);
      const proofFiles = await prepareOrganizerProofFiles(selectedFiles);
      const payload = {
        ...normalizedData,
        proof_urls: [],
          ...
          <ol className="mt-7 space-y-4 border-t border-slate-100 pt-6">
            ...
            <li className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs leading-5 text-slate-500">
                  Bạn có thể đăng nhập để khám phá sự kiện trong lúc chờ. Quyền
                  quản lý sự kiện được mở sau khi hồ sơ được duyệt.
                </p>
              </div>
            </li>
          </ol>
