...
### 5.3. Đọc nội dung TỆP KẾ HOẠCH sự kiện (`events.file_url`) — bổ sung

**Bối cảnh:** khi Ban tổ chức tạo sự kiện, form (`EventForm.jsx` → `Card "Kế hoạch
sự kiện" required`) bắt buộc tải lên **1 tệp kế hoạch** (`.pdf` hoặc `.docx`) lên
bucket Storage `event_plan`; URL công khai được lưu vào cột `events.file_url`
(`REQUIRED_FOR_SUBMIT` trong `schemas/organizer_event.py`). Trước đây chatbox
**không dùng** tệp này — chỉ lấy 160 ký tự đầu của `description` — nên không trả
lời được các câu hỏi chi tiết (lịch trình, thể lệ, giải thưởng, yêu cầu chuẩn bị…).

**Đã thêm (chỉ ở `chatbot_service.py`, không thêm dependency):**

1. `_fetch_context_events()` lấy thêm cột `file_url`.
2. `_events_referenced(message, history, events)` — đoán câu hỏi đang nói về **sự
   kiện cụ thể nào** bằng cách khớp tiêu đề với câu hỏi + 4 lượt hội thoại gần nhất
   (khớp nguyên tiêu đề, hoặc ≥2 từ khoá có nghĩa / tiêu đề ≤2 từ khoá khớp hết).
   Chỉ khi nhận ra sự kiện cụ thể mới tải tệp → câu hỏi chung không tốn băng thông.
3. `_load_event_plan(file_url)` — tải tệp (qua `httpx`, có cache theo tiến trình,
   giới hạn `PLAN_MAX_BYTES = 12MB`):
   - `.docx` → trích văn bản bằng **thư viện chuẩn** (`zipfile` + `xml.etree`,
     đọc `word/document.xml`), cắt còn `PLAN_CONTEXT_MAX_CHARS = 6000` ký tự, đưa
     vào prompt dưới khối **“KẾ HOẠCH CHI TIẾT SỰ KIỆN”**.
   - `.pdf` → gửi **thẳng bytes** cho Gemini như một `inline_data` part
     (`mime_type: application/pdf`) — Gemini tự đọc PDF, không cần lib PDF. Tổng
     PDF đính kèm 1 request giới hạn `PLAN_PDF_TOTAL_BYTES = 14MB`.
4. `_build_contents(...)` nhận thêm `plan_context` + `plan_pdf_parts`; `SYSTEM_PROMPT`
   cho phép dùng khối “KẾ HOẠCH CHI TIẾT SỰ KIỆN” / tệp PDF đính kèm để trả lời,
   vẫn cấm bịa và yêu cầu nói “chưa có thông tin” nếu không có kế hoạch của đúng
   sự kiện đó.
5. Tối đa `PLAN_MAX_EVENTS = 2` sự kiện được nạp kế hoạch mỗi câu hỏi. Mọi lỗi
   tải/đọc tệp đều bị nuốt (log + bỏ qua) → không ảnh hưởng luồng trả lời chính.

**Biến môi trường mới (đều có mặc định, không bắt buộc):** `CHAT_PLAN_MAX_CHARS`,
`CHAT_PLAN_MAX_EVENTS`.
...
- **Tệp kế hoạch (`file_url`) chỉ nạp cho sự kiện nằm trong 25 sự kiện ngữ cảnh**
  (đã duyệt, công khai, chưa kết thúc). Hỏi kế hoạch của sự kiện đã kết thúc / chưa
  duyệt sẽ không có nội dung tệp. Việc nhận diện “đang hỏi sự kiện nào” dựa trên
  khớp tiêu đề — tiêu đề quá chung chung có thể trượt; khi đó trợ lý báo “chưa có
  thông tin” thay vì đoán.
- **Trích `.docx` bằng `zipfile`/`xml`** chỉ lấy văn bản đoạn (`<w:t>`) — bảng,
  header/footer, text-box có thể bị bỏ sót. `.pdf` do Gemini tự đọc nên không bị
  hạn chế này. PDF scan (ảnh, không có text layer) phụ thuộc khả năng OCR của model.
- **Cache tệp kế hoạch theo tiến trình** (`_PLAN_CACHE`, tối đa 24 mục, xoá sạch khi
  đầy): sửa nội dung tệp mà giữ nguyên URL thì bản cũ còn được dùng tới khi restart.
...
Sửa:
  ...
  src/backend/app/services/chatbot_service.py      (đọc nội dung tệp kế hoạch events.file_url —
                                                   .docx trích bằng zipfile/xml, .pdf gửi thẳng
                                                   cho Gemini; xem mục 5.3)
